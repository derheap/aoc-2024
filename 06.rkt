...
(define input (file->lines "input/06.txt"))
(define test_input
  (string-split
   (string-trim
    "....#.....
...
(define (find-guard-start ht)
  (map string->number
       (string-split (first (hash-keys (hash-filter-values
                                        ht
                                        (λ (v)
                                          (equal? v #\^)))))
                     ",")))
...
(define (is-obstacle-next-step grid dir x y)
  (is-obstacle? grid (next-pos-x dir x) (next-pos-y dir y)))
...
(define (next-step grid locations xmax ymax dir x y)
  (hash-set! locations
             (string-append-immutable (number->string x)
                                      ","
                                      (number->string y)
             ...
             dir)
  (if (is-obstacle-next-step grid dir x y)
      (step grid locations xmax ymax (turn-right dir) x y)
      (step grid
            locations
            xmax
            ymax
...
(define (step grid locations xmax ymax dir x y)
  (if (on-grid? xmax ymax x y)
      (next-step grid locations xmax ymax dir x y)
      locations))
...
(define (part#1 input)
  ...
  (length (hash-values
           (step grid locations xmax ymax #\N x y))))
...
(define (next-step-p2 grid locations xmax ymax dir x y)
  (hash-set! locations
             (string-append-immutable (number->string x)
                                      ","
                                      (number->string y)
...
(define (step-p2 grid locations xmax ymax dir x y)
  (cond
    [(is-looping? locations dir x y) #t]
    [(on-grid? xmax ymax x y)
     (next-step-p2 grid locations xmax ymax dir x y)]
    [else #f]))
...
(define (add-obstacle-and-test-looping grid  x y xmax ymax gx gy)
    (define locations (make-hash))
    (define test-grid (hash-copy grid))
    (add-obstacle test-grid x y)
    (if (step-p2 test-grid locations xmax ymax #\N gx gy) 1 0)
    )
   ...
   (for/list ([y (range ymax)])
     (apply
      ...
      (for/list ([x (range xmax)])
       (add-obstacle-and-test-looping grid x y xmax ymax gx gy)))))
  )
