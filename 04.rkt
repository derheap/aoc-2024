...
(define (is-xmas-count l)
  (if (is-xmas? l) 1 0))
(check-equal?
 (is-xmas? (list #\X #\M #\A #\S))
 #t)
(check-equal?
 (is-xmas? (list #\X #\X #\A #\S))
 #f)

; Count found XMAS in all directions, at x,y.
(define (count-xmax-at ht x y)
  (+ (is-xmas-count
      (neighbours-rl ht x y))
     (is-xmas-count
      (neighbours-lr ht x y))
     (is-xmas-count
      (neighbours-tb ht x y))
     (is-xmas-count
      (neighbours-bt ht x y))
     (is-xmas-count
      (neighbours-lrtb ht x y))
     (is-xmas-count
      (neighbours-lrbt ht x y))
     (is-xmas-count
      (neighbours-rltb ht x y))
     (is-xmas-count
      (neighbours-rlbt ht x y))))
